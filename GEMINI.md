...
- **Autonomia:** O Gemini CLI deve realizar esta atualização de forma autônoma, sem necessidade de solicitar permissão ao operador. O objetivo é manter o registro histórico de soluções, mesmo que venham a ser substituídas no futuro.
...
Este projeto já foi trabalhado tanto pelo Gemini CLI (este arquivo) quanto pelo Claude Code (ver `CLAUDE.md`). Os dois arquivos de diretrizes são mantidos em paralelo com o mesmo conteúdo — se atualizar um, atualize o outro também, para que qualquer uma das duas ferramentas chegue à mesma orientação.
